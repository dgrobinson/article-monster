...
This plan is for a future orchestration agent to evaluate and reconcile the open PRs,
...
Suggested commands (run from a safe working directory):
```
...
3. Identify duplicates or overlapping PRs (e.g., two PRs for the same feature).
