...
(function(window) {
  ...
  Readability.prototype = {
    ...
    _grabArticle: function(doc) {
      ...
      var articleBodies = doc.querySelectorAll('.article__body, [class*="article-body"], .body__content, .entry-content');
      if (articleBodies.length > 0) {
        var totalParagraphs = 0;
        for (var i = 0; i < articleBodies.length; i++) {
          totalParagraphs += articleBodies[i].querySelectorAll('p').length;
        }
        
        console.log('Found', articleBodies.length, 'article body container(s) with', totalParagraphs, 'total paragraphs');
        
        if (totalParagraphs > 5) {
          // If multiple containers, combine them
          if (articleBodies.length > 1) {
            var container = doc.createElement('div');
            for (var j = 0; j < articleBodies.length; j++) {
              var clone = articleBodies[j].cloneNode(true);
